...
public class ScoreBiz implements IScoreBiz{
    ...
    public ScoreAck deductScore(ScoreReq scoreReq) {
        try {
            ...
            scoreChangeRecordService.insertSelective(scoreChange);
            return ScoreAck.renderSuccess(ScoreCode.SCORE000001, scoreReq.getOrderCode());
    ...
    private int insertScoreChild(ScoreChild scoreChild) {
        int result = scoreChildService.insertScoreChild(scoreChild);
        if (1 != result) {
            throw new ScoreException(ExceptionEnum.SCORE_CHILD_INSERT_EXCEPTION, "新增积分子账户失败!");
        }
        return result;
    }
    ...
    private void _validateScoreReqForProduce(ScoreReq scoreReq) {
        _validateScoreReqForBase(scoreReq);
        Assert.hasText(scoreReq.getIncomeAccount(), "请求参数scoreReq对应的incomeAccount不能为空!");
    }
    ...
    public ScoreAck produceScore(ScoreReq scoreReq) throws ScoreException{
        try {
            ...
            //查询积分账户是否已开启
            Score score = scoreService.getScoreByUserId(scoreReq.getIncomeAccount());
            Date time = new Date();
            ...
            if (null != score) {
                ...
                ScoreChild scoreChild = scoreChildService.getScoreChildByUserIdAndExpirationTime(params);
                if (null != scoreChild) {
                ...
                scoreChildService.insertScoreChild(scoreChild);
            ...
            throw new ScoreException(ExceptionEnum.PARAM_CHECK_EXCEPTION, "produceScore参数校验错误!");
        } catch (Exception e) {
            logger.error("produceScore发生错误!", e);
            throw new ScoreException(ExceptionEnum.SYSTEM_EXCEPTION, "produceScore发生错误!" + scoreReq);
        }
    }
    
    private Score _createScore(ScoreReq scoreReq, Date time) {
        //获取score_auth权限，如果是卖家，生成积分账号为卖家类型，否则为买家类型
        Auth tmpAuth = authService.getAuthByUserId(scoreReq.getIncomeAccount());
        String userType = "";
        if (tmpAuth != null && tmpAuth.getShopId().intValue() != 0) {
            userType = Score.ScoreUserType.SELLER.toString();
        } else if (tmpAuth != null && tmpAuth.getShopId().intValue() == 0) {
            throw new BusinessException(ExceptionEnum.PARAM_CHECK_EXCEPTION, "平台管理员不会产生积分账户!");
        } else {
            userType = Score.ScoreUserType.BUYER.toString();
        }
        Score score = new Score(scoreReq.getIncomeAccount(), userType, scoreReq.getScore(), scoreReq.getChannelCode(), time);
        return score;
    }
